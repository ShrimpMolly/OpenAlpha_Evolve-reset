...
class TaskManagerAgent(TaskManagerInterface):
    def __init__(self, task_definition: TaskDefinition, config: Optional[Dict[str, Any]] = None):
        ...
        self.prompt_designer: PromptDesignerInterface = PromptDesignerAgent(
        ...
        if settings.DATABASE_TYPE == "sqlite":
            if SQLiteDatabaseAgent:
                ...
                logger.error(
                    "SQLiteDatabaseAgent is not available. Falling back to InMemoryDatabaseAgent.")
            ...
            logger.warning(
                f"Unknown DATABASE_TYPE: {settings.DATABASE_TYPE}. Defaulting to in-memory database.")
            self.database: DatabaseAgentInterface = InMemoryDatabaseAgent()
    ...
    async def initialize_population(self) -> List[Program]:
        logger.info(
            f"Initializing population for task: {self.task_definition.id}")
        initial_population = []
        ...
        for i in range(self.population_size):
            ...
            logger.debug(
                f"Generating initial program {i+1}/{self.population_size} with id {program_id}")
        ...
        self.selection_controller.initialize_islands(
            initial_programs=initial_population)

        logger.info(
            f"Initialized population with {len(initial_population)} programs across {self.num_islands} islands.")
    ...
    async def evaluate_population(self, population: List[Program]) -> List[Program]:
        ...
        evaluated_programs = []
        evaluation_tasks = [self.evaluator.evaluate_program(
            prog, self.task_definition) for prog in population if prog.status != "evaluated"]
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                logger.error(
                    f"Error evaluating program {original_program.id}: {result}", exc_info=result)
        ...
        logger.info(
            f"Finished evaluating population. {len(evaluated_programs)} programs processed.")
        return evaluated_programs

    async def manage_evolutionary_cycle(self):
        logger.info(
            f"Starting evolutionary cycle for task: {self.task_definition.description[:50]}...")
        ...
        for gen in range(1, self.num_generations + 1):
            ...
            parents = self.selection_controller.select_parents(
                current_population, self.num_parents_to_select)
            if not parents:
                logger.warning(
                    f"Generation {gen}: No parents selected. Ending evolution early.")
                break
            logger.info(f"Generation {gen}: Selected {len(parents)} parents.")

            # Generate offspring
            offspring_population = []
            num_offspring_per_parent = (
                self.population_size + len(parents) - 1) // len(parents)

            generation_tasks = []
            for i, parent in enumerate(parents):
                for j in range(num_offspring_per_parent):
                    # Always use a UUID for every new offspring to guarantee global uniqueness
                    child_id = f"{self.task_definition.id}_gen{gen}_child_{uuid.uuid4().hex[:8]}"
                    generation_tasks.append(
                        self.generate_offspring(parent, gen, child_id))
            ...
            for result in generated_offspring_results:
                if isinstance(result, Exception):
                    logger.error(
                        f"Error generating offspring: {result}", exc_info=result)
                elif result:
                    offspring_population.append(result)
                    await self.database.save_program(result)

            logger.info(
                f"Generation {gen}: Generated {len(offspring_population)} offspring.")
            if not offspring_population:
                logger.warning(
                    f"Generation {gen}: No offspring generated. May indicate issues with LLM or prompting.")
                if not parents:
                    break
            ...
            current_population = self.selection_controller.select_survivors(
                current_population, offspring_population, self.population_size)
            logger.info(
                f"Generation {gen}: New population size: {len(current_population)}.")

            # Log best program in this generation
            # Use PRIMARY_OBJECTIVE for sorting
            if settings.PRIMARY_OBJECTIVE == "runtime_ms":
                best_program_this_gen = sorted(
                    current_population,
                    key=lambda p: (p.fitness_scores.get("runtime_ms", float(
                        'inf')), p.fitness_scores.get("correctness", 0)),
                )
            else:
                best_program_this_gen = sorted(
                    current_population,
                    key=lambda p: (p.fitness_scores.get(
                        "correctness", -1), -p.fitness_scores.get("runtime_ms", float('inf'))),
                    reverse=True
                )
            if best_program_this_gen:
                logger.info(
                    f"Generation {gen}: Best program: ID={best_program_this_gen[0].id}, Fitness={best_program_this_gen[0].fitness_scores}")
            else:
                logger.warning(
                    f"Generation {gen}: No programs in current population after survival selection.")
                break

        logger.info("Evolutionary cycle completed.")
        # Use settings.PRIMARY_OBJECTIVE for final best selection
        sort_order = "asc" if settings.PRIMARY_OBJECTIVE == "runtime_ms" else "desc"
        final_best = await self.database.get_best_programs(task_id=self.task_definition.id, limit=1, objective=settings.PRIMARY_OBJECTIVE, sort_order=sort_order)
        if final_best:
            logger.info(
                f"Overall Best Program: {final_best[0].id}, Code:\n{final_best[0].code}\nFitness: {final_best[0].fitness_scores}")
        else:
    ...
    async def generate_offspring(self, parent: Program, generation_num: int, child_id: str) -> Optional[Program]:
        logger.debug(
            f"Generating offspring from parent {parent.id} for generation {generation_num}")
        ...
        if parent.errors and parent.fitness_scores.get("correctness", 1.0) < settings.BUG_FIX_CORRECTNESS_THRESHOLD:
            ...
            logger.info(
                f"Attempting bug fix for parent {parent.id} using diff. Error: {primary_error}")
            prompt_type = "bug_fix"
        else:
            feedback = {
            ...
            mutation_prompt = self.prompt_designer.design_mutation_prompt(
                program=parent, evaluation_feedback=feedback)
            logger.info(
                f"Attempting mutation for parent {parent.id} using diff.")
        ...
        if not generated_code.strip():
            logger.warning(
                f"Offspring generation for parent {parent.id} ({prompt_type}) resulted in empty code/diff. Skipping.")
            return None

        if generated_code == parent.code:
            logger.warning(
                f"Offspring generation for parent {parent.id} ({prompt_type}) using diff resulted in no change to the code. Skipping.")
            return None

        if "<<<<<<< SEARCH" in generated_code and "=======" in generated_code and ">>>>>>> REPLACE" in generated_code:
            logger.warning(
                f"Offspring generation for parent {parent.id} ({prompt_type}) seems to have returned raw diff. LLM or diff application may have failed. Skipping. Content:\n{generated_code[:500]}")
            return None
        ...
        logger.info(
            f"Successfully generated offspring {offspring.id} from parent {parent.id} ({prompt_type}).")
        return offspring
...
if __name__ == '__main__':
    ...
    sample_task = TaskDefinition(
        ...
        evaluation_criteria={
            "target_metric": "correctness", "goal": "maximize"},
    ...
    async def run_task():
        try:
            ...
            logger.error(
                "An error occurred during the task management cycle.", exc_info=True)
