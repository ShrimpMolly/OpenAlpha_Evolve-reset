...
async def run_evolution(
    ...
    try:
        ...
        allowed_imports = [imp.strip()
                           for imp in allowed_imports_text.split(",") if imp.strip()]
        ...
        async def progress_callback(generation, max_generations, stage, message=""):
            ...
            progress(min(total_progress, 0.99),
                     f"Generation {generation}/{max_generations}: {message}")

            logger.info(
                f"Progress: Generation {generation}/{max_generations} - {message}")
        ...
        with gr.Column(scale=1):
            with gr.Tab("Results"):
                results_text = gr.Markdown(
                    "Evolution results will appear here...")

    example_btn.click(
        set_fib_example,
        outputs=[task_id, description, function_name,
                 examples_json, allowed_imports]
    )
